...
$data = getenv('DefaultConnection') .' '.getenv('SENDGRID_APIKEY').' '.getenv('CLOUDINARY_DETAILS');
...
$myFile = '/tmp/redeploy_docker.txt'; # output 
...
//                 throw new \Exception('This push time has expired, kindly create another push ' . $hookSecret);
//   }
...
//                 // echo 'Thanks';
                exec('sudo /home/agent/hooks/redeploy-github-hook.sh '.$data.' > /dev/null & 2>&1', $output);
//                 exec('printenv sudo -E /home/agent/hooks/redeploy-github-hook.sh 2>&1 > /dev/null &', $output);
                 $fh = fopen($myFile, 'w');

                 if (is_array($output)) { fwrite($fh, print_r($output, TRUE)); } else { fwrite($fh, $output); }
